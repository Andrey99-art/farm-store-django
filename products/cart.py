...
class Cart:
    ...
    def __iter__(self):
        ...
        for item in cart.values():
            ...
            item['quantity'] = Decimal(item['quantity']) # <-- Конвертируем в Decimal для вычислений
            item['total_price'] = item['price'] * item['quantity']
            yield item
    def __len__(self):
        # __len__ обычно ожидает целое число, поэтому здесь могут быть нюансы.
        # Давайте считать количество уникальных позиций в корзине.
        return len(self.cart.keys())
    
    def get_total_quantity(self):
        # Отдельный метод для подсчета общего количества (может быть дробным)
